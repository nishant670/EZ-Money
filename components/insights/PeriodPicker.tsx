...
export type PeriodPreset = 'this_month' | 'last_month' | 'last_7_days' | 'last_30_days' | 'last_3_months' | 'custom';
...
export function PeriodPicker({ visible, onClose, onSelect, currentRange }: PeriodPickerProps) {
    ...
    const [selectedPreset, setSelectedPreset] = useState<PeriodPreset | undefined>(currentRange.preset);
...
function PresetsView({ selectedPreset, onSelectPreset, onGoToCustom, onApply, onClose, theme }: any) {
    return (
            ...
            <ScrollView contentContainerStyle={styles.presetsList} showsVerticalScrollIndicator={false}>
                {PRESETS.map((p) => {
                    ...
                    return (
                        ...
                        >
                            <Text style={[styles.presetText, { color: theme.text }, isSelected && { color: theme.text }]}>{p.label}</Text>
                            {isSelected && (
...
function CustomRangeView({ start, end, onSelectStart, onSelectEnd, onBack, onConfirm, theme }: any) {
    ...
    return (
        <View style={styles.viewContainer}>
            <View style={styles.header}>
                ...
                <Text style={[styles.title, { color: theme.text, flex: 1, textAlign: 'center' }]}>Select Custom Range</Text>
                <View style={{ width: 40 }} />
            </View>

            <View style={styles.dateDisplays}>
                <View style={[styles.dateBox, start && !end && { borderColor: theme.accent, borderWidth: 1 }]}>
                    <Text style={styles.dateLabel}>START DATE</Text>
                    <Text style={[styles.dateValue, { color: theme.text }]}>
                        {start ? start.toLocaleDateString('default', { month: 'short', day: '2-digit', year: 'numeric' }) : 'Select Date'}
                    </Text>
                </View>
                <View style={[styles.dateBox, end && { borderColor: theme.accent, borderWidth: 1 }]}>
                    ...
                    <Text style={[styles.dateValue, { color: theme.text }]}>
                        {end ? end.toLocaleDateString('default', { month: 'short', day: '2-digit', year: 'numeric' }) : 'Select Date'}
                    </Text>
                </View>
            </View>
